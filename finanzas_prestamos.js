...
function setLang(lang) {
    ...
    document.querySelectorAll('[data-es]').forEach(element => {
        ...
        // Si contiene otros elementos traducibles,
        // no modificar directamente su contenido.
    ...
    document.querySelectorAll('img[data-img-es]').forEach(img => {
        ...
        switch (lang) {
            ...
            case 'es':
                nuevaImagen = img.getAttribute('data-img-es');
                break;

            case 'en':
                nuevaImagen = img.getAttribute('data-img-en');
                break;

            case 'pt':
                nuevaImagen = img.getAttribute('data-img-pt');
                break;

            case 'fr':
                nuevaImagen = img.getAttribute('data-img-fr');
                break;

            case 'al':
                nuevaImagen = img.getAttribute('data-img-al');
                break;

            case 'zh':
                nuevaImagen = img.getAttribute('data-img-zh');
                break;
        ...
        // Si la imagen del idioma no existe,
        // usar la imagen en español.
        if (!nuevaImagen) {
            nuevaImagen =
                img.getAttribute('data-img-es');
        }
    ...
    document
        .querySelectorAll('[data-lang-code]')
        .forEach(element => {
    ...
    document
        .querySelectorAll('[data-lang-flag]')
        .forEach(currentFlag => {
...
// ============================================
// SOLICITUD DE PRÉSTAMO — ACORDEÓN ACCESIBLE
// ============================================

function sincronizarEstadoSolicitud(boton) {
    ...
    const estaCerrada =
        tarjeta.classList.contains('esta-cerrada');
...
function igualarCajasSolicitud() {
    ...
    tarjetas.forEach(tarjeta => {
        ...
        // Si está cerrada, dejar que CSS controle la altura
        if (
...
function iniciarSolicitudPrestamo() {
    ...
    if (!botonesSolicitud.length) return;
                ...
                document
                    ...
                    .forEach(otraTarjeta => {
                        ...
                        if (
                            ...
                            otraTarjeta.classList.add(
                                'esta-cerrada'
                            );


                            const otroBoton =
                                otraTarjeta.querySelector(
                                    '.solicitud-pregunta'
                                );
...
const PREGUNTAS_POR_IDIOMA = {
    ...
    pt: [
        ...
        {
            pregunta: "O que é um cartão de crédito?",
            resposta: "É um meio de pagamento que permite comprar agora e pagar depois."
        },
        {
            pergunta: "O que é um fundo de emergência?",
            respuesta: "É um dinheiro reservado para despesas inesperadas."
        },
        {
            pergunta: "Por que investir é importante?",
            resposta: "Investir pode ajudar seu dinheiro a crescer com o tempo e alcançar objetivos financeiros."
        },
        {
        ...
        },
        {
            ...
            resposta: "Juros compostos são juros calculados sobre o dinheiro inicial e também sobre os juros acumulados."
        },
        {
            pergunta: "O que é histórico de crédito?",
            resposta: "É um registro de como você administra suas dívidas e pagamentos."
        },
        {
            pergunta: "Como posso começar a poupar?",
            resposta: "Comece acompanhando suas despesas, definindo uma meta e guardando uma quantia regularmente."
        },
        {
