...
function setLang(lang) {
    ...
    // El buscador de podcasts tiene su propio placeholder por idioma
    // (setLang solo traduce textContent, no atributos)
    actualizarPlaceholderPodcast(lang);
...
function setActiveNavLink() {
    ...
    document.querySelectorAll('nav a').forEach(link => {
        ...
        if (dropdownPadre) {
            const linkPadre = dropdownPadre.querySelector(':scope > a');
            if (linkPadre) linkPadre.classList.add('active');
        }
    });
}
...
document.querySelectorAll('nav a').forEach(link => {
    link.addEventListener('click', () => {
        ...
        if (dropdownPadre) {
            const linkPadre = dropdownPadre.querySelector(':scope > a');
            if (linkPadre && linkPadre !== link) linkPadre.classList.add('active');
        }
    });
});
...
(() => {
    const grupos = document.querySelectorAll('.podcast-grid');
    ...
    grupos.forEach(grupo => {
        Array.from(grupo.children).forEach((tarjeta, index) => {
            tarjeta.style.setProperty('--animation-delay', `${(index % 6) * 80}ms`);
...
(() => {
    ...
    document.querySelectorAll('a, button, .podcast-card, #strix-btn').forEach(element => {
...
// ============================================================
// PODCASTS — buscador y filtro por idioma del audio
// ============================================================

function filtrarPodcasts() {
    const tarjetas = document.querySelectorAll('.podcast-card');
    const vacio = document.getElementById('podcastEmpty');
    if (!tarjetas.length) return;

    const query = (document.getElementById('podcastSearch')?.value || '').trim().toLowerCase();
    const filtroActivo = document.querySelector('.podcast-filtro.is-active')?.dataset.filtro || 'todos';

    let visibles = 0;

    tarjetas.forEach(card => {
        const coincideIdioma = filtroActivo === 'todos' || card.dataset.lang === filtroActivo;
        const texto = card.textContent.toLowerCase();
        const coincideBusqueda = !query || texto.includes(query);
        const visible = coincideIdioma && coincideBusqueda;
...
// El placeholder del buscador se traduce aparte porque setLang solo
// toca textContent, no atributos como "placeholder".
function actualizarPlaceholderPodcast(lang) {
    const input = document.getElementById('podcastSearch');
    if (!input) return;
    const key = `${lang}Placeholder`;
    input.placeholder = input.dataset[key] || input.dataset.esPlaceholder;
}

document.querySelectorAll('.podcast-filtro').forEach(boton => {
    boton.addEventListener('click', () => {
        document.querySelectorAll('.podcast-filtro').forEach(b => b.classList.remove('is-active'));
        boton.classList.add('is-active');
        filtrarPodcasts();
    });
});

const inputBusquedaPodcast = document.getElementById('podcastSearch');
if (inputBusquedaPodcast) {
    inputBusquedaPodcast.addEventListener('input', filtrarPodcasts);
}
