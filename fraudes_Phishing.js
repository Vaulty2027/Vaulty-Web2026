// ============================================
// VAULTY+ — FRAUDES / PHISHING
// Misma lógica que index.js: IDIOMAS + NAV + MENÚ
// + ANIMACIONES + STRIX + SCROLL + CURSOR
// ============================================
...
function setLang(lang) {
    ...
    // ----------------------------------------
    // IMÁGENES CON VERSIÓN DISTINTA SEGÚN EL IDIOMA
    // (ej. el mapa de "Cómo protegerte")
    // ----------------------------------------
    document.querySelectorAll('[data-img-es]').forEach(img => {
        const key = `img${lang.charAt(0).toUpperCase()}${lang.slice(1)}`; // dataset.imgEs, imgEn, imgPt...
        img.src = img.dataset[key] || img.dataset.imgEs;
    });
...
(() => {
    const grupos = document.querySelectorAll('.cards-grid, .tipos-grid, .entidades-lista, .mitos-tabla');
...
(() => {
    ...
    document.querySelectorAll('a, button, .herramientas-card, #strix-btn').forEach(element => {
