...
(() => {
    const grupos = document.querySelectorAll('.cards-grid, .features-grid, .entidades-lista, .cards-container, .hero-features, .recaudacion-grid, .planning-grid');
    ...
    grupos.forEach(grupo => {
        Array.from(grupo.children).forEach((tarjeta, index) => {
            tarjeta.style.setProperty('--animation-delay', `${index * 100}ms`);
...
// ============================================
// PARALLAX DEL HERO (si esta página tiene .hero-inicio-img)
// ============================================

(() => {
    const heroImg = document.querySelector('.hero-inicio-img');
    if (!heroImg) return;
    ...
    function actualizarHero() {
        const desplazamiento = Math.min(window.scrollY * 0.25, 120);
        heroImg.style.transform = `translateY(${desplazamiento}px) scale(1.08)`;
        frame = null;
    }

    window.addEventListener('scroll', () => {
        if (frame !== null) return;
        frame = requestAnimationFrame(actualizarHero);
    }, { passive: true });
})();
...
(() => {
    ...
    document.querySelectorAll('a, button, .entidad-card, .card, .recaudacion-card, .planning-card, #strix-btn').forEach(element => {
...
// ============================================
// FRASE ROTATIVA DEL HERO (solo si existe #heroTaglineWord)
// ============================================

const TAGLINES_POR_IDIOMA = {
    es: ['Ahorra con propósito.', 'Invierte con confianza.', 'Crece sin miedo.', 'Aprende a tu ritmo.'],
    en: ['Save with purpose.', 'Invest with confidence.', 'Grow without fear.', 'Learn at your pace.'],
    fr: ['Épargnez avec un but.', 'Investissez en confiance.', 'Grandissez sans crainte.', 'Apprenez à votre rythme.'],
    pt: ['Poupe com propósito.', 'Invista com confiança.', 'Cresça sem medo.', 'Aprenda no seu ritmo.'],
    al: ['Sparen mit Ziel.', 'Investieren mit Vertrauen.', 'Wachsen ohne Angst.', 'Lernen im eigenen Tempo.'],
    zh: ['有目标地储蓄。', '自信地投资。', '无畏地成长。', '按自己的节奏学习。']
};
...
function reiniciarTagline() {
    const contenedor = document.getElementById('heroTaglineWord');
    if (!contenedor) return;

    const frases = TAGLINES_POR_IDIOMA[idiomaActual] || TAGLINES_POR_IDIOMA.es;
    taglineIndex = 0;

    if (taglineTimer) clearInterval(taglineTimer);

    contenedor.textContent = frases[0];
    contenedor.classList.remove('leaving');

    const reducirMovimiento = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (reducirMovimiento || frases.length < 2) return;

    taglineTimer = setInterval(() => {
        contenedor.classList.add('leaving');
        setTimeout(() => {
            taglineIndex = (taglineIndex + 1) % frases.length;
            contenedor.textContent = frases[taglineIndex];
            contenedor.classList.remove('leaving');
...
document.addEventListener('DOMContentLoaded', reiniciarTagline);
...
document.querySelectorAll('.faq-item').forEach(item => {
    ...
    question.addEventListener('click', () => {
        const estaAbierta = item.classList.contains('open');

        document.querySelectorAll('.faq-item.open').forEach(abierta => {
            if (abierta !== item) {
                abierta.classList.remove('open');
                const btn = abierta.querySelector('.faq-question');
                if (btn) btn.setAttribute('aria-expanded', 'false');
            }
        });

        item.classList.toggle('open', !estaAbierta);
        question.setAttribute('aria-expanded', String(!estaAbierta));
    });
});
...
const PREGUNTAS_POR_IDIOMA = {
    ...
    zh: [
        { pregunta: "什么是预算？", respuesta: "预算是一个帮助你organize收入和支出、更好地管理资金的计划。" },
...
const retireYear = document.getElementById('retireYear');
const retireMessage = document.getElementById('retireMessage');
const ringProgress = document.querySelector('.ring-progress');

let generoActual = 'M'; // M o F

// Datos de traducciones para el mensaje dinámico
const mensajes = {
    es: { futuro: "Sigue planificando tu futuro financiero.", listo: "¡Ya alcanzaste la edad de jubilación!" },
    en: { futuro: "Keep planning your financial future.", listo: "You have reached the retirement age!" },
    fr: { futuro: "Continuez à planifier votre avenir financier.", listo: "Vous avez atteint l'âge de la retraite !" },
    pt: { futuro: "Continue planejando seu futuro financeiro.", listo: "Você já atingiu a idade de aposentadoria!" },
    al: { futuro: "Planen Sie weiter Ihre finanzielle Zukunft.", listo: "Sie haben das Renteneintrittsalter erreicht!" },
    zh: { futuro: "继续规划您的财务未来。", listo: "您已达到退休年龄！" }
};

function calcularJubilacion() {
    if (!ageSlider) return;

    const edad = parseInt(ageSlider.value);
    const edadRetiro = generoActual === 'M' ? 62 : 57;
    const anosRestantes = edadRetiro - edad;
    const anioActual = new Date().getFullYear();

    // Actualmer textos básicos
    ageValue.textContent = edad;
    retireAge.textContent = edadRetiro;

    // Detectar idioma actual (leyendo el atributo lang del HTML o del localStorage)
    let lang = localStorage.getItem('lang') || 'es';
    if (!mensajes[lang]) lang = 'es';

    // Lógica de resultados
    if (anosRestantes > 0) {
        yearsLeft.textContent = anosRestantes;
        retireYear.textContent = anioActual + anosRestantes;
        retireMessage.textContent = mensajes[lang].futuro;
    } else {
        yearsLeft.textContent = "0";
        retireYear.textContent = anioActual;
        retireMessage.textContent = mensajes[lang].listo;
    }

    // Animar el anillo SVG
    // El progreso es la edad actual sobre la edad de retiro (max 100%)
    const progreso = Math.min(edad / edadRetiro, 1);
    const circunferencia = 597; // 2 * PI * 95
...
if (ageSlider) {
    ageSlider.addEventListener('input', calcularJubilacion);
}

// Ejecutar al cargar
document.addEventListener('DOMContentLoaded', calcularJubilacion);