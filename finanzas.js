...
function setActiveNavLink() {
    ...
    document.querySelectorAll('nav a').forEach(link => {
        ...
        // Si este link vive dentro de un submenú y coincide con la
        // página actual, el link padre (el que abre el submenú)
        // también se marca activo — igual que en el resto del sitio.
        if (href === currentPage) {
            const padre = link.closest('.nav-item.dropdown');
            if (padre) {
...
(() => {
    const grupos = document.querySelectorAll('.finanzas-cards');
...
(() => {
    const heroImg = document.querySelector('.hero-finanzas-img');
    if (!heroImg) return;
    ...
    function actualizarHero() {
        const desplazamiento = Math.min(window.scrollY * 0.2, 100);
...
(() => {
    ...
    document.querySelectorAll('a, button, .finanzas-card, #strix-btn').forEach(element => {
...
// (funcionalidad propia de esta página; se mantiene tal cual estaba)
// ============================================
