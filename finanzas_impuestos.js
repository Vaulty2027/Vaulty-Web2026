...
function setLang(lang) {
    ...
    // ----------------------------------------
    // CAMBIAR IMÁGENES POR IDIOMA (data-img-es / data-img-en / ...)
    // ----------------------------------------
    document.querySelectorAll('[data-img-es]').forEach(img => {
        const src = img.dataset['img' + lang.charAt(0).toUpperCase() + lang.slice(1)] || img.dataset.imgEs;
        if (src) img.src = src;
    });
...
// (se agregaron acá los contenedores propios de esta página:
//  tipos-grid, quien-lista, ruta-financiera, consejos-grid,
//  fondo-grid, formas-ahorrar-grid, entidades-bancos-grid)
// ============================================

(() => {
    const grupos = document.querySelectorAll(
        '.cards-grid, .features-grid, .entidades-lista, .cards-container, .hero-features, ' +
        '.tipos-grid, .quien-lista, .ruta-financiera, .consejos-grid, .fondo-grid, ' +
        '.formas-ahorrar-grid, .entidades-bancos-grid'
    );
...
(() => {
    ...
    document.querySelectorAll('a, button, .entidad-card, .tipo-card, .entidad-banco-card, #strix-btn').forEach(element => {
...
// ============================================
// STRIX — PREGUNTAS POR IDIOMA (mismas que index.js)
// ============================================
