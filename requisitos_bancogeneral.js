...
function setLang(lang) {
    ...
    // (por ejemplo <strong> o el ícono de "Regresar") para no perder
    // el énfasis ni los iconos al traducir frases largas.
    // ----------------------------------------
...
function setActiveNavLink() {
    ...
    const paginasDePrestamos = ['finanzas_prestamos.html', 'requisitos_globalbank.html', 'requisitos_bancogeneral.html'];
...
// ============================================
// ANIMACIONES BANCO GENERAL (reveal al hacer scroll)
// ============================================

function iniciarAnimacionesBancoGeneral() {
    const section = document.querySelector('.bancogeneral-section');
...
// ============================================
// MICROINTERACCIONES (spotlight en tarjetas + tasas animadas)
// ============================================

function iniciarMicrointeraccionesBancoGeneral() {
    ...
    const rateNumbers = document.querySelectorAll('.rate-option strong');
    ...
    const targets = [...rateNumbers].map(number =>
        Number.parseFloat(number.textContent) || 0
    );
...
// ============================================
// BOTÓN REGRESAR
// Usa el historial si existe (por ejemplo si el usuario llegó
// navegando desde "Préstamos"); si no hay historial (entrada
// directa por URL o pestaña nueva), cae siempre a la página de
// respaldo indicada en data-target para que el botón nunca falle.
// ============================================
...
function iniciarBotonRegresar() {
    ...
    const destinoRespaldo = backButton.dataset.target || 'finanzas_prestamos.html';

    backButton.addEventListener('click', () => {
        window.dispatchEvent(new CustomEvent('bancogeneral:back'));

        const vieneDelMismoSitio =
            document.referrer &&
            new URL(document.referrer).origin === window.location.origin;

        if (window.history.length > 1 && vieneDelMismoSitio) {
            window.history.back();
        } else {
            window.location.href = destinoRespaldo;
        }
    });
}
...
const PREGUNTAS_POR_IDIOMA = {
    ...
    es: [
        ...
        { pregunta: "¿Qué es un fondo de emergencia?", respuesta: "Es dinero reservado para cubrir gastos inesperados, como una enfermedad, una reparación o la pérdida de empleo." },
        { pregunta: "¿Por qué es importante invertir?", respuesta: "Invertir permite que tu dinero pueda crecer con el tiempo, ayudándote a alcanzar metas financieras a largo plazo." },
        { pregunta: "¿Qué es la inflación?", respuesta: "Es el aumento generalizado de los precios con el tiempo, lo que hace que tu dinero pierda poder de compra." },
        { pregunta: "¿Qué es el interés compuesto?", respuesta: "Es cuando ganas intereses no solo sobre tu dinero inicial, sino también sobre los intereses ya acumulados." },
        { pregunta: "¿Qué es el historial crediticio?", respuesta: "Es un registro de cómo has manejado tus deudas y pagos. Un buen historial te ayuda a acceder a préstamos con mejores condiciones." },
        { pregunta: "¿Cómo puedo empezar a ahorrar?", respuesta: "Empieza por anotar tus gastos, define una meta clara y separa un porcentaje fijo de tus ingresos apenas los recibas." },
        { pregunta: "¿Qué diferencia hay entre ahorrar e invertir?", respuesta: "Ahorrar es guardar dinero de forma segura. Invertir es poner tu dinero a trabajar para que crezca, aunque implica cierto riesgo." }
    ],
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    iniciarAnimacionesBancoGeneral();
    iniciarMicrointeraccionesBancoGeneral();
