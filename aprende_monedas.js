...
function setActiveNavLink() {
    ...
    document.querySelectorAll('nav a').forEach(link => {
        ...
        if (href === currentPage) {
            const padre = link.closest('.nav-item.dropdown');
            if (padre) {
                const trigger = padre.querySelector(':scope > a');
...
(() => {
    ...
    document.querySelectorAll('a, button, .coin-card, .info-card, #strix-btn').forEach(element => {
...
function inicializarRevelado() {
    const elementosAAnimar = document.querySelectorAll(
     ...
     '.conversor-section .conversor-container'
    );
...
// ============================================
// "TINTINEO" DE MONEDA AL PASAR EL MOUSE
// ============================================
    ...
    const observadorPasos = new IntersectionObserver((entradas) => {
        entradas.forEach(entrada => {
            ...
            if (entrada.isIntersecting) {
                ...
                icono.style.color = 'var(--navy-950)';
                icono.style.boxShadow = '0 0 0 6px rgba(217, 164, 65, 0.18)';
            } else {
                icono.style.background = 'var(--navy-950)';
...
// ============================================
// TARJETA 3D DEL CONVERSOR 
// ============================================

function inicializarTiltConversor() {
    const tieneMouse = window.matchMedia('(hover: hover) and (pointer: fine)').matches;
    const reducirMovimiento = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (!tieneMouse || reducirMovimiento) return;
...
// ============================================
// CONVERSOR DE MONEDAS — LÓGICA
// ============================================

const TASAS_DE_CAMBIO = {
    USD: { USD: 1, EUR: 0.92, MXN: 18.45, GBP: 0.79, JPY: 149.50, CNY: 7.24 },
    EUR: { USD: 1.09, EUR: 1, MXN: 20.05, GBP: 0.86, JPY: 162.50, CNY: 7.87 },
    MXN: { USD: 0.054, EUR: 0.050, MXN: 1, GBP: 0.043, JPY: 8.10, CNY: 0.39 },
    GBP: { USD: 1.27, EUR: 1.16, MXN: 23.35, GBP: 1, JPY: 189.20, CNY: 9.18 },
    JPY: { USD: 0.0067, EUR: 0.0062, MXN: 0.12, GBP: 0.0053, JPY: 1, CNY: 0.048 },
    CNY: { USD: 0.138, EUR: 0.127, MXN: 2.54, GBP: 0.109, JPY: 20.75, CNY: 1 }
};
...
function inicializarConversor() {
    ...
    if (!amountFrom || !amountTo || !currencyFrom || !currencyTo || !rateDisplay) return;

    function calcularConversion() {
        const monto = parseFloat(amountFrom.value) || 0;
        const monedaOrigen = currencyFrom.value;
        const monedaDestino = currencyTo.value;

        if (!TASAS_DE_CAMBIO[monedaOrigen] || !TASAS_DE_CAMBIO[monedaOrigen][monedaDestino]) {
            amountTo.value = '0.00';
            return;
        }

        const tasa = TASAS_DE_CAMBIO[monedaOrigen][monedaDestino];
        const resultado = monto * tasa;

        const opciones = { minimumFractionDigits: 2, maximumFractionDigits: 2 };
        amountTo.value = new Intl.NumberFormat('en-US', opciones).format(resultado);
        rateDisplay.textContent = `1 ${monedaOrigen} = ${tasa.toFixed(4)} ${monedaDestino}`;
    }

    function intercambiarMonedas() {
        const tempMoneda = currencyFrom.value;
        currencyFrom.value = currencyTo.value;
        currencyTo.value = tempMoneda;
        calcularConversion();
    }

    amountFrom.addEventListener('input', calcularConversion);
    currencyFrom.addEventListener('change', calcularConversion);
    currencyTo.addEventListener('change', calcularConversion);
    if (swapBtn) swapBtn.addEventListener('click', intercambiarMonedas);
