...
(() => {
    const grupos = document.querySelectorAll(
        '.cards-grid, .features-grid, .entidades-lista, .cards-container, .hero-features, .finanzas-cards'
    );
...
// ============================================
// EFECTO RIPPLE — botón de cada tarjeta (.card-title)
// ============================================
...
(() => {
    ...
    document.querySelectorAll('.card-title').forEach(boton => {
        boton.addEventListener('click', event => {
