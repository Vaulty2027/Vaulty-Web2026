...
(() => {
    ...
    const observer = new IntersectionObserver(entries => {
        entries.forEach(entry => {
            ...
            // Dejar de observar después de aparecer.
...
(() => {
    const grupos = document.querySelectorAll(
        '.content-grid, .consequence-grid, .loan-types-grid, .related-grid'
    );
    ...
    grupos.forEach(grupo => {
        ...
        Array.from(grupo.children).forEach((tarjeta, index) => {
            ...
            // Agregar la clase antes de que el observer
            // comience a observar las tarjetas.
            tarjeta.classList.add('reveal-scale');

            // Retraso de cada tarjeta.
            tarjeta.style.setProperty(
                '--animation-delay',
                `${index * 80}ms`
            );

            tarjeta.style.transitionDelay = `${index * 60}ms`;
        });
    });

    // Si se reducen las animaciones, mostrar las tarjetas.
    if (reducirMovimiento) {
        grupos.forEach(grupo => {
            grupo.querySelectorAll('.reveal-scale').forEach(tarjeta => {
    ...
    // Observar las tarjetas después de agregar
    // la clase .reveal-scale.
    const tarjetas = document.querySelectorAll('.reveal-scale');
...
(() => {
    ...
    document.querySelectorAll('a, button, .info-card, .related-card, #strix-btn').forEach(element => {
